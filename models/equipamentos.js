...
class Equipamento {
    ...
    listaDepartamento(res, id){
        const sql ='SELECT *FROM EQUIPAMENTOS WHERE DEPARTAMENTO_FK =?';
        conexao.query(sql,[id], (erro, resultado) =>{
            if(erro){
                res.status(400).json(erro);
            }else{
    ...
    atualizaStatus(id, equipamento, res){
        const sql = 'update equipamentos set status = ? where id = ?';
        conexao.query(sql, [equipamento.status, id], (erro, resultado)=>{
            if(erro){
               return;
            }else{
               return;
